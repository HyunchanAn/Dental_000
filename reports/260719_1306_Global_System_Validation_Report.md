# Global System Validation Report (260719_1306)
...
| **Dental_002** | 파노라마 X-ray에서 **치아 자체에 발생하는 병소(충치, 매복치 등)**를 전담하여 검출하는 YOLOv8 기반 모듈입니다.... | 0.9 | 0.89 | 0.92 |
...
| **Dental_008** | 파노라마 X-ray에서 개별 치아를 Instance Segmentation하고, FDI 시스템(치식)을 매칭하는 모듈입니다.... | 0.98 | N/A | N/A |
...
| **Dental_012** | 이 레포지토리는 파노라마 방사선 사진에서 **치근단 병소(Periapical Lesion)** 를 자동으로 탐지하고 분류하는 딥러닝 기반 모듈을... | 0.86 | 0.85 | 0.88 |
| **Dental_013** | 파노라마 방사선 사진에서 분할(Segmentation)된 개별 치아 이미지를 입력받아, 해당 치아의 수복물 및 보철물 상태(Crown, Impl... | 0.95 | 0.94 | 0.96 |
