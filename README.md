...
## 📊 진단 모듈별 실측 성능 지표 (Diagnostic Performance Metrics)
...
| Dental_013 | 치과 수복물 및 보철물 (Restoration) | 88.5% | 84.2% | 수복물/인레이/크라운/아말감 분류 (EfficientNet/YOLO) |
| Dental_014 | 하악 피질골 기반 골다공증 위험도 | 73.1% (C3) | 17.6% (특이도) | 하악 하연 피질골 지수(MCW) 딥러닝 스크리닝 |

> [지표 가이드]: 객체 탐지 및 분할 모듈(008, 002, 012, 003, 013)은 진음성(TN) 면적 정의가 불분명하여 특이도 대신 임상적 유효성을 대변하는 정밀도(Precision)로 산출되었습니다. 영상 전체 분류 모듈인 014는 특이도(Specificity)를 직접 기재하였습니다.
