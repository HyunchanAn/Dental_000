...
reader_path = os.path.abspath(r'C:\Users\chema\Github\Dental_Panoramic_Reader')
...
def run_test():
    img_path = os.path.join(reader_path, 'test_images_PANO', 'panoramic_12.jpg')
    ...
    print(f"002 Caries Data found: {len(results.get('002_lesions', {}).get('boxes', []))} lesions")
    print(f"012 Periapical Data found: {len(results.get('012_periapical', {}).get('lesions', []))} lesions")
    print(f"013 Restoration Data found: {len(results.get('013_restorations', {}).get('lesions', []))} lesions")
