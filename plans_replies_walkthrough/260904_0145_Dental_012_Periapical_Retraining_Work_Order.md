...
# [ 260904 2355 메인 워크스테이션 사용자 UAT 피드백 접수 및 마스킹/라벨링 왜곡 3대 원인 정밀 규명 보고서 ]

사용자(치과의사 전문의)가 랩탑 현장 환경에서 `Dental_015` 및 서빙 파이프라인을 직접 가동하여 검증한 결과, "마스킹 및 라벨링 위치가 영 딴 곳으로 가있다"는 결정적 결함이 접수되었습니다. 이에 따라 전체 작업을 잠정 중단하고 레포지토리를 커밋/푸시한 후, 메인 워크스테이션에서 긴급 코드 역추적 감사를 집행하여 다음과 같이 3대 기술적 원인을 100% 명확히 규명하였습니다.

---

## 1. 마스킹 및 라벨링 위치 왜곡의 3대 치명적 원인 규명

### 1. [핵심 결함] 512x512 패치 좌표의 전역 역투영 스케일링 누락 (`dental_002.py`)
- 버그 위치: `Dental_Panoramic_Reader/core/interfaces/dental_002.py`
- 현상:
  - `Dental_002` 모델은 512x512로 정규화된 치아 패치 영상에서 BBox 좌표(`rx1, ry1, rx2, ry2`)를 추론합니다.
  - 이를 원래 패치의 가로세로 픽셀 크기(`pw, ph`)로 환산하는 `rx1 / 512.0 * pw` 정규화 스케일링을 거치지 않고 `gx1 = px1 + rx1`로 단순 덧셈을 적용하였습니다.
- 결과:
  - 예를 들어 실제 치아 패치 너비가 80픽셀인데 모델이 패치 내 400픽셀 지점에 박스를 치면, 파노라마 전역에서 박스가 5배나 거대화되어 파노라마 전체 너비의 26%(W: 0.26) x 높이의 73%(H: 0.73)에 달하는 거대한 괴물 박스가 턱뼈와 허공 전체를 덮어버리는 참사가 발생했습니다.

### 2. [데이터 누락] 치아 분할(008) 마스크와 FDI 치아 번호(11~48) SSOT 전송 부재
- 버그 위치: `Dental_Panoramic_Reader/api_server.py` 및 `Dental_015/src/types/finalReport.ts`
- 현상:
  - 백엔드 `format_to_ssot_report`가 `Dental_008`에서 검출된 치아 32개의 BBox와 FDI 번호를 프론트엔드로 전달하는 규격(`findings.teeth`)을 아예 누락하였습니다.
  - 프론트엔드 `PanoramaCanvasViewer.tsx` 역시 개별 치아의 번호나 마스크를 렌더링하는 코드가 전혀 없어, 충치 박스 위에 엉뚱하게 `null Caries`나 대략적인 사분면 근사치(`Quadrant Approx`)만 표출되었습니다.

### 3. [시각 왜곡] 캔버스 종횡비 2:1 고정으로 인한 Y축 찌그러짐
- 버그 위치: `Dental_015/src/components/PanoramaCanvasViewer.tsx`
- 현상:
  - 캔버스 해상도가 `1200x600`(2:1 고정)으로 하드코딩되어 있어, 실제 2.3:1 비율의 파노라마 이미지가 캔버스에 그려질 때 세로로 억지로 늘어나면서 실제 치아의 해부학적 위치와 바운딩 박스 간의 시각적 불일치가 발생했습니다.

---

## 2. 차기 세션 긴급 조치 계획 (Next Action Plan)

1. `dental_002.py` 좌표 역투영 정밀 스케일링 공식 적용:
   - `gx1 = px1 + (rx1 / 512.0) * pw`
   - `gy1 = py1 + (ry1 / 512.0) * ph`
   - `gx2 = px1 + (rx2 / 512.0) * pw`
   - `gy2 = py1 + (ry2 / 512.0) * ph`
2. 백엔드/프론트엔드 치아(Teeth) SSOT 스키마 정식 탑재:
   - `finalReport.ts`에 `teeth?: Array<{ fdi: number; box: NormalizedBBox }>` 정의.
   - `api_server.py`에서 `008_tooth_data`의 치아 32개 BBox 및 FDI 번호를 `findings.teeth`로 정식 전달.
3. `PanoramaCanvasViewer.tsx` 치아 번호 뱃지 렌더링 완결:
   - 각 치아 상단/하단에 깔끔한 FDI 번호 태그 표출 및 이미지 원본 종횡비 자동 반응형 캔버스 뷰어로 개선.

---

본 원인 분석 결과를 확정 기록하고 현 세션을 종료 및 마감합니다.